...
# Graceful import to prevent immediate app crashes on Windows systems
# missing the required GTK3/Pango C-libraries.
try:
    from weasyprint import HTML
    WEASYPRINT_AVAILABLE = True
except (OSError, ImportError) as e:
    WEASYPRINT_AVAILABLE = False
    WEASYPRINT_ERROR_MSG = str(e)
...
def generate_tech_pdf(report_title: str, metrics: dict, plot_data, battery_enabled: bool) -> io.BytesIO:
    ...
    if not WEASYPRINT_AVAILABLE:
        raise RuntimeError("The PDF rendering engine is missing required system libraries (GTK3/Pango). Please check the server environment.")
